...
func testCharacterToOneGameUsingGame(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, characterDBTypes, true, characterColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Character struct: %s", err)
	}
	...
	slice := CharacterSlice{&local}
	if err = local.L.LoadGame(ctx, tx, false, (*[]*Character)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Game == nil {
...
func testCharacterToOneSetOpGameUsingGame(t *testing.T) {
	var err error
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, characterDBTypes, false, strmangle.SetComplement(characterPrimaryKeyColumns, characterColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
...
func testCharacterToOneRemoveOpGameUsingGame(t *testing.T) {
	var err error
	...
	if err = a.SetGame(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveGame(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.Game().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
...
var (
	characterDBTypes = map[string]string{`ID`: `uuid`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `Name`: `character varying`, `GameID`: `uuid`}
	_                = bytes.MinRead
)
...
func testCharactersUpdate(t *testing.T) {
	...
	if len(characterAllColumns) == len(characterPrimaryKeyColumns) {
...
func testCharactersSliceUpdateAll(t *testing.T) {
	...
	var fields []string
	if strmangle.StringSliceMatch(characterAllColumns, characterPrimaryKeyColumns) {
		fields = characterAllColumns
	} else {

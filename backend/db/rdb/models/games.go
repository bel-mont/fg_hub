...
var GameWhere = struct {
	...
	Slug:      whereHelperstring{field: "\"games\".\"slug\""},
}
...
type gameR struct {
	Characters CharacterSlice `boil:"Characters" json:"Characters" toml:"Characters" yaml:"Characters"`
}
...
func (r *gameR) GetCharacters() CharacterSlice {
	if r == nil {
		return nil
	}
	return r.Characters
...
var (
	gameAllColumns            = []string{"id", "created_at", "updated_at", "name", "slug"}
	gameColumnsWithoutDefault = []string{"slug"}
	gameColumnsWithDefault    = []string{"id", "created_at", "updated_at", "name"}
...
// Characters retrieves all the character's Characters with an executor.
func (o *Game) Characters(mods ...qm.QueryMod) characterQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"characters\".\"game_id\"=?", o.ID),
	)

	return Characters(queryMods...)
}

// LoadCharacters allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (gameL) LoadCharacters(ctx context.Context, e boil.ContextExecutor, singular bool, maybeGame interface{}, mods queries.Applicator) error {
	var slice []*Game
	var object *Game
	...
	query := NewQuery(
		qm.From(`characters`),
		qm.WhereIn(`characters.game_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load characters")
	}

	var resultSlice []*Character
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice characters")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on characters")
	}
	...
	if singular {
		object.R.Characters = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.GameID) {
				local.R.Characters = append(local.R.Characters, foreign)
				if foreign.R == nil {
					foreign.R = &characterR{}
				}
...
// AddCharacters adds the given related objects to the existing relationships
// of the game, optionally inserting them as new records.
// Appends related to o.R.Characters.
// Sets related.R.Game appropriately.
func (o *Game) AddCharacters(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Character) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.Characters = append(o.R.Characters, related...)
	}
...
// SetCharacters removes all previously related items of the
// game replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Game's Characters accordingly.
// Replaces o.R.Characters with related.
// Sets related.R.Game's Characters accordingly.
func (o *Game) SetCharacters(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Character) error {
	query := "update \"characters\" set \"game_id\" = null where \"game_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.Characters {
	...
	return o.AddCharacters(ctx, exec, insert, related...)
}

// RemoveCharacters relationships from objects passed in.
// Removes related items from R.Characters (uses pointer comparison, removal does not keep order)
// Sets related.R.Game.
func (o *Game) RemoveCharacters(ctx context.Context, exec boil.ContextExecutor, related ...*Character) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("game_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.Characters {
			if rel != ri {
				continue
			}

			ln := len(o.R.Characters)
			if ln > 1 && i < ln-1 {
				o.R.Characters[i] = o.R.Characters[ln-1]
			}
			o.R.Characters = o.R.Characters[:ln-1]
			break
		}
	}
